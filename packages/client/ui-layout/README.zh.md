...
AppFrame 始终挂载会话栏和详情栏；已连接 Session 通过 `SessionProvider` 渲染。布局 store 是瞬时状态，两栏均以默认宽度启动；AULOS 将 Workspace Files 树放入详情栏，活跃会话默认显示该栏，且 store 从不读写 `localStorage`。hero 与 blank-session 状态会把详情栏的渲染宽度派生为零，但不改变存储的宽度偏好。Files 栏跨会话切换保持打开；用户可通过标题栏开关或面板自身的隐藏按钮关闭它（`ctx.layout.toggleDetails`）。会话 owner share 为空，侧边栏 owner share 只包含 `collapsed` 和 `width`；注册方通过标准钩子获取业务数据，并从各自的 inject 接口获取操作。
...
- **面板几何信息是瞬时状态**：重新加载会恢复两栏默认值；未选中与 blank-session 表面以零宽度渲染详情栏，但不会修改存储的宽度偏好。

...
AppFrame always mounts the conversation and details columns; a connected Session renders through `SessionProvider`. The transient layout store starts both panels at their default widths — the details column is the right inspector and is open by default for active sessions — and it never reads or writes `localStorage`. Hero and blank-session states derive a zero rendered details width without changing that stored preference. The inspector stays open across session switches; the user closes it with the header toggle or the panel's own hide button (`ctx.layout.toggleDetails`). The conversation owner share is empty, while the sidebar owner share contains only `collapsed` and `width`; registrants obtain business data from standard hooks and actions from their own inject faces.

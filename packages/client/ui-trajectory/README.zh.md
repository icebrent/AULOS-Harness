...
同一个共享 `TrajectoryBody` 在两个界面渲染，数据路径也相同（`snapshot.views.get('trajectory')`）：完整页面视图标签（`conversation.view`，id `trajectory`）与聊天底部面板（`conversation.session.bottom`）。完整标签是详细的 debug／audit 界面，从会话标题栏的工具按钮打开；底部面板默认是一根收起条，展开后成为可拖拽调高的面板（160–560px，拖动顶部手柄），sticky 停靠在编辑器正上方，聊天与记录表因此同时可见。聊天工具行的 inspect 请求会展开面板并把目标调用交给共享 body；完整标签活跃时面板卸载，记录表绝不渲染两次。两个界面共用同一个 inject face：翻页、实际时长偏好与全部 renderer 状态在 body 挂载的任何位置都一致。

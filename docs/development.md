...
The repository has no hosted CI workflows; contributors run the relevant local gates before pushing. [scripts/run-gates.ts](../scripts/run-gates.ts) owns the current gate and job inventory, and `pnpm run test:e2e` exercises the real-API suite locally with its configured worker bound.

...
describe('web e2e: settled Markdown math rendering', () => {
  ...
  it.skipIf(MODE === 'record')('renders the settled reply without KaTeX errors', async () => {
    ...
    // The restored stats strip under the composer carries the whole-log
    // turn/step counts (the sessionStats projection) — one turn, one step.
    await expect.poll(() => page.getByText('1 turns · 1 steps').count(), { timeout: 10_000 }).toBe(1)

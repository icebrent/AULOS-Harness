...
在 `packages/host/` 落一个三包能力 seam——`directory-picker`（Service Definition）、`directory-picker-native`、`directory-picker-browse`（后端）——唯一约定方法 `capability()` 返回**可辨识联合**：`{ kind: 'native', pick(signal), list(path?, signal?) }` 或 `{ kind: 'browse', list(path?, signal?), createDirectory(path, name) }`。网关（`dsh-host-apiproxy`）注入 `directoryPicker`，提供对应的 RPC，没有能力变体承载的请求以 `directory-picker-unavailable` 应答。两个变体虽都提供 `list`，联合仍保持可辨识：`pick` 仍为 native 专属（宿主屏幕上的 OS 选择器）、`createDirectory` 仍为 browse 专属——压平成统一方法集仍会逼每个后端伪装另一方的形态。
...
- **列举层级有上限且流式处理，引擎归 seam 所有、两个后端共享。** 引擎位于 Service Definition 包（`directory-picker/src/listing.ts`，导出为 `listDirectoryLevel`），因此 `native` 能力提供的 `list` 与 `browse` 后端遵循完全相同的约定：自适应选择器按宿主处境解析后端（回环 Windows 宿主解析为 `native`），Files 树消费方必须在该后端下照常工作——没有任何消费方为列举去读 `capability().kind`。每个后端保留各自的 `Config.maxEntries`（默认 1000——GitHub 网页端目录列举的同一上限）并传给引擎。单次 `list` 至多返回 `maxEntries` 行。层级经 `opendir` 流入一个按名排序、容量 `maxEntries + 1` 的候选窗口，内存保持 O(maxEntries)，可进入性探测只触及窗口内候选；线上 `DirectoryListing` 携带必填的 `truncated` 标志，让客户端明示不完整而不是静默缺尾。窗口内的断链符号链接不从窗口外回填——发生过驱逐本身已把层级标记为截断。窗口插入使用二分查找、满窗尾部单次比较即拒绝（超大层级不能为每个 dirent 付出一次全窗扫描），且 `list(path, signal)` 透传载体的请求信号，滞塞网络目录的扫描不会在调用方断连后继续存活——扫描中的每个 await（打开、每次读取、每次符号链接探测）都与信号赛跑，中止路径放弃而非等待 close（Node 会把 close 排在在飞读取之后），被放弃的 settlement 全部吞掉，清理不会以未处理拒绝的形式冒出。无上限的层级对超大或恶意构造的目录就是内存／响应性漏洞。
- **网关从任何携带 `list` 的后端提供列举。** `host.listDirectory` 在解析出的能力上执行、不做 kind 检查，只对完全不提供列举原语的后端（未来的合并扩展 kind）以 `directory-picker-unavailable` 应答，拒绝路径保持覆盖；`host.pickDirectory` 与 `host.createDirectory` 保持各自的 native-only／browse-only 守卫。
...
- **统一的 Service Definition 方法集（`pick(): path`）。** 否决：应用内浏览器无法藏在一次宿主侧调用后面——浏览循环在客户端，需要协议上的原语；压平的方法集仍会逼原生选择器伪装 browse 专属的创建原语、浏览器伪装宿主屏幕上的 pick。交互差异不可约，故用判别标签。

...
it('lets a preset producer reach the background-job registry', async () => {
  ...
  try {
    ...
    expect({ isError: started.isError, content: started.content }).toEqual({
      isError: false,
      content: [{ type: 'text', text: `started background job ${LIVE_SHELL_TOOL}-1` }],
    })
    ...
    expect(listed.content).toEqual([
      { type: 'text', text: expect.stringContaining(`${LIVE_SHELL_TOOL}-1 [${LIVE_SHELL_TOOL}]`) as unknown as string },
    ])
    ...
    const collected = await ctx.tools.execute({
      ...
      name: 'job_output',
      arguments: { job_id: `${LIVE_SHELL_TOOL}-1`, wait: true },
      agent: handle.agent,
    })

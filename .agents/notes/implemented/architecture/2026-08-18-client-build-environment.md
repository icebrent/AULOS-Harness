...
The root build wrapper supplies one exact public environment to both bundlers. It derives `DSH_CLIENT_COMMIT_HASH` as the seven-character prefix of the source Git HEAD for every complete build; an explicit value supports build environments without repository metadata. `pnpm run build` and `pnpm run build:aulos` select the AULOS artifact profile, set `DSH_CLIENT_BUILD_PROFILE=aulos`, and set the AULOS browser title. `pnpm run build:local` inherits the caller's `DSH_CLIENT_*` values, while `pnpm run build:official` selects the repository's official artifact profile and sets `DSH_CLIENT_BUILD_PROFILE=official`. These scripts require no shell-specific environment syntax. A successful complete build writes the exact public environment and a digest covering the Vite output and every dynamic client bundle. Partial build commands do not replace that record.

...
每条 Session 行采用两行布局：标题与相对时间之下，是由列表投影值折算出的安静元数据行，包含计费输入 token、缓存命中率和上下文占用率。`sessionRowMetrics` 从会话界面使用的同一组 `tokenUsage` 与 `contextPressure` 投影推导这些值，因此侧边栏不解析会话日志；没有投影值的行保持单行高度。

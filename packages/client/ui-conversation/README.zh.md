...
会话领域：骨架（标题栏／编辑器／空状态）、聊天视图（分组步骤摘要流、流式尾部隔离与轮次状态）、transcript 上方的紧凑上下文卡（占用率／token／缓存／性能，全部来自官方投影）、底部轨迹区、编辑器 dock（与输入区一同 sticky 的会话统计行）、输入区 dock（队列行加 todo 计划条），以及按 scope 寻址的 ConversationController。工具展示属于 [`ui-tool`](../ui-tool/README.md)；右栏属于 [`ui-files`](../ui-files/README.md) 的项目文件树。
...
聊天视图保留工具的消息流位置，但委托其展示。每个已排序的 `tool-call` Conversation Node 都通过 `conversation.chat.node` 的同名 key 分发。组装后的 Web bundle 为该 Chat Node key 注册 [`ui-tool`](../ui-tool/README.md)，由后者渲染运行时已投影的递归 root/child 树，并负责按名称分发、通用展示和 render-intent 卡片。选中或 inspect 一个调用只会高亮该行或打开轨迹面（底部面板／完整标签）；详情面板已不存在。
...
逐会话 UI 状态中的选择与活跃视图位于已声明的聊天 store（`stores.ts` `createChatStore`）中；InputHub 拥有输入区状态机，并将草稿镜像到该 store 以便持久化。apply 将同一个 store handle 传给严格限定于会话的子树、其页头、聊天视图与「完整轨迹」工具按钮注册，因此每个会话内共享一个实例，框架拥有其生命周期。组件保持纯粹：框架标准工具包提供 `useSession`／`sessionId`、全局 `useSessions`／`useWorkspaces`，以及输入状态机的 `useInput`／`inputActions`；store 表层与 inject factory 提供其余状态和回调。
...
- **工具调用选中不再有详情面板**：被移除的右侧 Inspector 带走了原始调用展示（Input/Output/Metadata）；聊天行选中仅做高亮，完整的参数／结果检查位于轨迹面（底部面板与完整标签）。预览或编辑器面需要新的 seam。
- **底部轨迹面板在切换会话时丢失展开与高度状态**：面板状态是组件本地的，重挂载会恢复默认收起；持久化需要逐会话 store。

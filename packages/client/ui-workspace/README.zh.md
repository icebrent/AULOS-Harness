...
每条 Session 行都是两行：标题与相对时间，其下是安静的元数据行——由列表发布的投影值折算出的计费输入 token（紧凑格式）、缓存命中率与上下文占用率（带细进度条）。`sessionRowMetrics` 从对话界面同样读取的 `tokenUsage`／`contextPressure` 投影推导这些数字，因此侧边栏从不解析会话日志；没有投影值的行保持历史单行高度。

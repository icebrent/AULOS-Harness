...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'root',
    summary: 'The mode selector under the workspace picker on the new-session screen: two primary entries (Chat / Code) plus a More menu for every other preset.',
    doc: 'The mode selector under the workspace picker on the new-session\nscreen: two primary entries (Chat / Code) plus a More menu for every\nother preset. Root scope: no session exists yet, so the choice is\nstaged for the next one rather than applied to a current one.',
    registerOptions: [],
    ownerProps: [
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'conversation\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-brand-aulos AulosBrandMark',
  ...
  {
    key: 'conversation.hero.subtitle',
    kind: 'single',
    scope: 'root',
    summary: 'Optional secondary copy under the blank-session title.',
    doc: 'Optional secondary copy under the blank-session title. The shell owns\ntypography and renders no second line when no occupant registers.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'conversation\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-brand-aulos AulosHeroSubtitle',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.hero.subtitle\', () => ctx.slots.register(\n      { name: \'conversation.hero.subtitle\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-conversation/src/client/contract/slots.ts:193',
  },
  {
    key: 'conversation.hero.title',
    kind: 'single',
    scope: 'root',
    summary: 'Primary copy in the blank-session headline.',
    doc: 'Primary copy in the blank-session headline. The shell owns typography\nand supplies its localized product headline when no occupant registers.',
    registerOptions: [],
    ownerProps: [
      '/** Presentation props supplied to a blank-session brand-copy occupant. */\nexport interface HeroBrandCopyOwnerProps {\n  /** Host CSS class that preserves the shell-owned typography. */\n  className?: string | undefined\n}',
    ],
    ...
    declaredBy: 'an entry in \'conversation\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-brand-aulos AulosHeroTitle',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.hero.title\', () => ctx.slots.register(\n      { name: \'conversation.hero.title\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  {
    key: 'conversation.session.bottom',
    kind: 'single',
    scope: 'session',
    summary: 'The chat\'s bottom trajectory region, rendered by the session body entry (which shares the chat store with the view ring): one occupant (ui-trajectory\'s resizable panel), so the trajectory body and the chat are visible at once.',
    doc: 'The chat\'s bottom trajectory region, rendered by the session body\nentry (which shares the chat store with the view ring): one occupant\n(ui-trajectory\'s resizable panel), so the trajectory body and the chat\nare visible at once. The owner dispatches the same\nConvViewOwnerProps inspect handoff as the view ring — a chat\ntool-row inspect expands the panel — and renders the slot only while\nthe full trajectory tab is inactive, so the same ledger never appears\ntwice. The region stays empty while no entry registers.',
    registerOptions: [],
    ownerProps: [
    ...
    occupants: [
      'client-ui-trajectory TrajectoryPanel',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'conversation.session.header\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-conversation FullTrajectoryButton id \'trajectory\'',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-brand-aulos AulosBrandName',
